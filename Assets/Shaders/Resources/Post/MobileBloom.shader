...
Shader "Hidden/FastBloom" {
	...
	CGINCLUDE
		...
		#define BLOOMLOG 3.0
		#define BLOOMLOGINV (1.0 / BLOOMLOG)
		...
		{	
			...
			color.rgb += tex2D(_Bloom, i.uv).rgb;
			color.a *= 1.0 / 0.98;
            ...
            fixed4 fragBloom3 ( v2f_simple i ) : SV_Target
            {
                fixed4 color = tex2D(_MainTex, i.uv);

                fixed stencil = max(0, 1.5 - color.a * 1.0);
                //return stencil;
                //color = color.a;
                //color = 0.0;
                fixed3 tmp = tex2D(_Bloom256, i.uv).rgb;


                fixed3 bloomcolor = 0;
                bloomcolor += tmp * tmp * 0.3 * stencil;// * fixed3(0,0,1);
                tmp = tex2D(_Bloom128, i.uv).rgb;
                bloomcolor += tmp * tmp * 0.3 * stencil;// * fixed3(0,0,1);
                tmp = tex2D(_Bloom64, i.uv).rgb;
                bloomcolor += tmp * tmp * 0.3 * stencil;// * fixed3(0,0,1);

                color.rgb += pow(bloomcolor, BLOOMLOGINV);
		...
		{				
			...
			color = pow(color, BLOOMLOG);

			return (color - _Parameter.z) * _Parameter.w;
			//return max(color / 4 - 0.25, 0) * ONE_MINUS_THRESHHOLD_TIMES_INTENSITY;
		}
